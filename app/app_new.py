...
def load_dataset():
    ...
    response,log = hh.get_ipfs("bafybeig7pplmikh3w43d2bp7r4lc24nspjljrjljutlsghvj6qnglxnxku")
...
def get_models(tw):
    if tw == 7:
        response,log = hh.get_ipfs("Qmc3jVcSprSaGgbUrQ2Y15Pk5KGheE9cDrqgSRJKvxhTms")
        load_ipfs_cids = hh.read_file(response.content).iloc[:st.session_state.model_count]
    elif tw == 14:
        response,log = hh.get_ipfs("QmR2GpkGTjdxLHa3uh8bfMULxyeTmwxn1nkQjHc6P6Pvnx")
        load_ipfs_cids = hh.read_file(response.content).iloc[:st.session_state.model_count]
    elif tw == 21:
        response,log  = hh.get_ipfs("QmYEb9bomnqLG8XXCr9k7VSCiEaj7htrXnvRW5etpm9YdQ")
        load_ipfs_cids = hh.read_file(response.content).iloc[:st.session_state.model_count]

    return load_ipfs_cids
...
def main(debug):
    ...
    df_cummul_amt = df_cummul.copy()[[col for col in df_cummul if col.endswith("_amt")]]

    df_cummul_amt.columns = [col.replace("_amt","") for col in df_cummul_amt]

    amt_melt = df_cummul_amt.rename(columns=col_mapping).reset_index().melt(id_vars="Date",var_name="Model",value_name="Amount")

    amt_melt["Model"] = amt_melt["Model"].str.replace("_amt","")


    df_pred = df_cummul[[col for col in df_cummul if col.endswith("_pred")]]
    df_pred.columns = [col.replace("_pred","") for col in df_pred]

    pred_melt = df_pred.rename(columns=col_mapping).reset_index().melt(id_vars="Date",var_name="Model",value_name="Pred")
    ...
    fig.update_layout(margin=dict(t=40),yaxis_title="Amount",
